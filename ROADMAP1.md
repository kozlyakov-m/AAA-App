...
4. **Аутентификация - 105 минут** 
 ...
 5. **Авторизация - 150 минут**
    ...
    6. Создать метод проверки существования роли в `BusinessLogic` (**R1.5**) - 10 минут
    7. Добавляем поля `res` и `role` в `ArgHandler` - 10 минут
    8. Добавляем метод проверки формата ресурса в `BusinessLogic` (**R1.3**) - 10 минут
        + Если неверный формат ресурса, заканчиваем выполнение программы с кодом 6
          Иначе продолжаем выполнение программы
    9. Добавляем метод поиска ресурса по имени в `BusinessLogic` (**R1.3**) - 10 минут
    10. Создать метод проверки доступа в `BusinessLogic` (есть ли у пользователя права на выбранную им роль) (**R1.3**) - 10 минут
