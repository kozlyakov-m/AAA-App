...
3. **Простейшие сценарии - 50 минут**
    ...
    1. Создать `class BuisnessLogic` - 10 минут
    2. Создать функцию вывода справки в `BuisnessLogic` - 10 минут
...
4. **Аутентификация - 105 минут** 
    1. Создать метод надо ли аутентифицировать в `BuisnessLogic` - 10 минут
        + (`args[0]=="-login" && args[2]=="-pass"`) (**R1.1**)
    2. Создать метод проверки правильности формата логина в `BuisnessLogic` (**R1.1**) - 10 минут
    3. Создать метод проверки существования логина в `BuisnessLogic` - 10 минут
         + `login==vasya`
    4. Создать метод проверки пароля в `BuisnessLogic` - 10 минут
         + `pass==123`
 ...
 5. **Авторизация - 150 минут**
    ...
    3. Создать `data class Permission` - 10 минут
        + `res`
        + `role`
        + `user`3
    4. Создать коллекцию ресурсов с тестовыми данными - 10 минут
    5. Создаем список допустимых ролей - 10 минут
    6. Создать метод проверки существования роли в `BuisnessLogic` (**R1.5**) - 10 минут
    7. Добавляем поля `res` и `role` в `ArgHandler` - 10 минут
    8. Добавляем метод проверки формата ресурса в `BuisnessLogic` (**R1.3**) - 10 минут
        + Если неверный формат ресурса, заканчиваем выполнение программы с кодом 6
          Иначе продолжаем выполнение программы
    9. Добавляем метод поиска ресурса по имени в `BuisnessLogic` (**R1.3**) - 10 минут
    10. Создать метод проверки доступа в `BuisnessLogic` (есть ли у пользователя права на выбранную им роль) (**R1.3**) - 10 минут
        + `user==vasya res==A role==READ`
    11. Переделать метод 5.10 на работу с коллекцией - 10 минут
    ...
    13. Создать метод авторизации в `BuisnessLogic` - 10 минут
...
6. **Аккаунтинг - 80 минут**
    ...
    2. Создаем метод нужен ли аккаунтинг в `BuisnessLogic`(**R1.11**) - 10 минут
    ...
    5. Создать метод проверки формата `ds` и `de` в `BuisnessLogic` - 10 минут
...
7. **Беспорядочная обработка аргументов - 80 минут**     
    ...
    методов `BuisnessLogic` на обработку всех методов и разложение их по полям - 20 минут
